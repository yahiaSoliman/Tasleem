"""
delivery flow for 2 orders per driver with one canceled order
verify that the other order can be delivered successfully
verify the status of canceled order
verify if canceled order is still linked to the driver
"""
...
"""
preconditions are:
1- have 1 idle driver in your store
"""
...
# cancel order
cancel_order(admin_access_token, order_id_a)
print("order " + str(order_id_a) + "is canceled")
# verify the order status
time.sleep(5)
order_status = get_order_status(admin_access_token, order_id_a)
assert order_status == "canceled", print("order status is " + str(order_status))
print("order status has been updated successfully to canceled")
...
assert current_orders_length == 1
current_order_id = current_orders[0]
assert current_order_id == order_id_b
